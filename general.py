    ...
    async def asklol(self, interaction: discord.Interaction, query: str):
        ...
        try:
            ...
            if intents['counters']:
                ...
                if counters:
                    # Separate into major (1000+) and off-meta (100-999)
                    major_counters = [c for c in counters if c.get('matches', 0) >= 1000]
                    offmeta_counters = [c for c in counters if 100 <= c.get('matches', 0) < 1000]
                    ...
                    # === MAJOR COUNTERS (1000+ games) - HARD MATCHUPS ===
                    if major_counters:
                        major_hard = major_counters[:5]  # Top 5 hardest
                        major_hard_text = ""
                        for c in major_hard:
                            ...
                            major_hard_text += f"{emoji} **{c_jp_name}** - {c['win_rate']}% WR ({matches:,}試合)\n"
                        
                        embed.add_field(
                            name=f"🛡️ メジャーカウンター - 不利{lane_display}\n📊 1000試合以上 | 使う時:BAN推奨 | 対面時:Pick推奨",
                            value=major_hard_text.strip(),
                            inline=False
                        )
                    
                    # === OFF-META COUNTERS (100-999 games) - HARD MATCHUPS ===
                    if offmeta_counters:
                        offmeta_hard = offmeta_counters[:5]  # Top 5 hardest
                        offmeta_hard_text = ""
                        for c in offmeta_hard:
                            matches = c.get('matches', 0)
                            norm_name = normalize_champion_name(c['name'])
                            c_jp_name = get_japanese_name(norm_name) if norm_name else c['name']
                            ...
                            offmeta_hard_text += f"{emoji} **{c_jp_name}** - {c['win_rate']}% WR ({matches}試合)\n"
                        
                        embed.add_field(
                            name=f"⚔️ オフメタカウンター - 不利{lane_display}\n📊 100-999試合 | 使う時:BAN推奨 | 対面時:Pick推奨",
                            value=offmeta_hard_text.strip(),
                            inline=False
                        )
                    
                    # === MAJOR COUNTERS - EASY MATCHUPS ===
                    if len(major_counters) >= 5:
                        major_easy = major_counters[-5:]  # Bottom 5
                        major_easy_text = ""
                        for c in major_easy:
                            matches = c.get('matches', 0)
                            norm_name = normalize_champion_name(c['name'])
                            ...
                            major_easy_text += f"{emoji} **{c_jp_name}** - {c['win_rate']}% WR ({matches:,}試合)\n"
                        
                        embed.add_field(
                            name=f"🛡️ メジャーカウンター - 有利{lane_display}\n📊 1000試合以上 | 使う時:Pick推奨 | 対面時:BAN推奨",
                            value=major_easy_text.strip(),
                            inline=False
                        )
                    
                    # === OFF-META COUNTERS - EASY MATCHUPS ===
                    if len(offmeta_counters) >= 5:
                        offmeta_easy = offmeta_counters[-5:]  # Bottom 5
                        offmeta_easy_text = ""
                        for c in offmeta_easy:
                            matches = c.get('matches', 0)
                            norm_name = normalize_champion_name(c['name'])
                            c_jp_name = get_japanese_name(norm_name) if norm_name else c['name']
                            ...
                            offmeta_easy_text += f"{emoji} **{c_jp_name}** - {c['win_rate']}% WR ({matches}試合)\n"
                        
                        embed.add_field(
                            name=f"⚔️ オフメタカウンター - 有利{lane_display}\n📊 100-999試合 | 使う時:Pick推奨 | 対面時:BAN推奨",
                            value=offmeta_easy_text.strip(),
                            inline=False
                        )
                else:
